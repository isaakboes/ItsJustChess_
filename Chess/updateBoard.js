        ...
        if ((tileX>=0&&tileX<boardSizeX)&&(tileY>=0&&tileY<boardSizeY)){//checks if the click is inside the board
            ...
            if (board[tileY][tileX].piece !== null){//if the board has a piece at the selected position
                ...
                if(board[tileY][tileX].movable){//if the current selected piece can move to the clicked tile
                    ...
                    //castling \/ \/ \/
                    if(board[tileY][tileX].piece instanceof Rook && board[tileY][tileX].piece.team == activeTeam && board[selectedPiece[1]][selectedPiece[0]].piece instanceof King){//if the board at the clicked position is a king and a rook
                       
                        if(selectedPiece[0]>tileX){//rook positions
                ...
                if(board[tileY][tileX].piece.team == activeTeam){//if the piece selected is the active team's piece, then select it
                    board[tileY][tileX].piece.selected = true;//selects the piece
                    selectedPiece = [tileX,tileY];//marks the pos of the selected piece and stores it
                }
                ...
                if(board[tileY][tileX].movable){
                    ...
                    unselectAll(true);
                    try{//our lord and savior, the try catch statement. the solver of problems!
                    ...
                    if (absolute){
                        ...
                        if(repeated){                          
                            if ((tileX+repeatedMoveX>=0&&tileX+repeatedMoveX<boardSizeX)&&(tileY+repeatedMoveY>=0&&tileY+repeatedMoveY<boardSizeY)){//checking if the move is in bounds
                                //console.log("in bounds at ("+(tileX+repeatedMoveX)+", "+(tileY+repeatedMoveY)+")")
                                ...
                                if ((tileX+repeatedMoveX>=0&&tileX+repeatedMoveX<boardSizeX)&&(tileY+repeatedMoveY>=0&&tileY+repeatedMoveY<boardSizeY)){//checking if the move is in bounds
                                    ...
                                    if(canTake||mustTake){//allows taking pieces on a move
                                        ...
                                        //try{//the wonderful try catch 
                                        if((tileX+repeatedMoveX>=0&&tileX+repeatedMoveX<boardSizeX)&&(tileY+repeatedMoveY>=0&&tileY+repeatedMoveY<boardSizeX)){

                                            //console.log(board[tileY+repeatedMoveY][tileX+repeatedMoveX].piece);
                                            try{
                                                if((board[tileY+repeatedMoveY][tileX+repeatedMoveX].piece !== null)){//if a piece exists at the desired location (for only attack moves)
                                                    
                                                    console.log("attacking '"+board[tileY+repeatedMoveY][tileX+repeatedMoveX].piece.team+"' at ("+(tileX+repeatedMoveX)+", "+(tileY+repeatedMoveY)+")")
                                                    if((canTakeOwnPawns && board[tileY][tileX].piece instanceof Pawn) || board[tileY+repeatedMoveY][tileX+repeatedMoveX].piece.team != team){//if the piece is another team or you can take your own pieces
                                                        console.log("can take '"+board[tileY+repeatedMoveY][tileX+repeatedMoveX].piece.team+"' at ("+(tileX+repeatedMoveX)+", "+(tileY+repeatedMoveY)+")")

                                                        board[tileY+repeatedMoveY][tileX+repeatedMoveX].movable = true;
                                                        board[tileY+repeatedMoveY][tileX+repeatedMoveX].highlighted = true;
                                                    }else{
                                                        console.log("cannot take '"+board[tileY+repeatedMoveY][tileX+repeatedMoveX].piece.team+"' at ("+(tileX+repeatedMoveX)+", "+(tileY+repeatedMoveY)+")")
                                                    }
                                                }
                                            }catch{console.log("piece undefined at ("+(tileX+repeatedMoveX)+", "+(tileY+repeatedMoveY)+")")}
                                        }
                                        //}catch{}
                                    }
                            ...
                            if ((tileX+moveX>=0&&tileX+moveX<boardSizeX)&&(tileY+moveY>=0&&tileY+moveY<boardSizeY)){//checking if the move is in bounds
                                ...
                                if(mustTake){//if the piece must take
                                    ...
                                    //en passant
                                    if(board[tileY+moveY+1][tileX+moveX].piece instanceof Pawn && (board[tileY+moveY+1][tileX+moveX].piece.team !== activeTeam || canTakeOwnPawns)){//if a pawn is selected and en passant is available
    ...
    for(let y=0;y<board.length;y++){
        for(let x=0;x<board[y].length;x++){
            ...
            if(board[y][x].discovered){
                ...
                if(!board[y][x].isMine){
                    switch(board[y][x].nearbyMines){//rendering tiles 
                        default:
                        case 0:ctx.drawImage(minesweeper.zero,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 1:ctx.drawImage(minesweeper.one,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 2:ctx.drawImage(minesweeper.two,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 3:ctx.drawImage(minesweeper.three,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 4:ctx.drawImage(minesweeper.four,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 5:ctx.drawImage(minesweeper.five,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 6:ctx.drawImage(minesweeper.six,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 7:ctx.drawImage(minesweeper.seven,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                        case 8:ctx.drawImage(minesweeper.eight,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);break;
                    }
                    //ctx.fillText(board[y][x].nearbyMines,x*tileSize+boardOffsetX+20,y*tileSize+boardOffsetY+20);
                }else{
                    ctx.drawImage(minesweeper.mine,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);
                    //ctx.fillText("Discovered Mine",x*tileSize+boardOffsetX+20,y*tileSize+boardOffsetY+20);
                }
            
            }
            if(board[y][x].exploded && board[y][x].isMine){
                ctx.drawImage(minesweeper.exploded,x*tileSize+boardOffsetX,y*tileSize+boardOffsetY,tileSize,tileSize);
            }
            ...
            if(board[y][x].piece !== null){//drawing pieces
                // if(Math.floor((mouseX-boardOffsetX)/tileSize) == x && Math.floor((mouseY-boardOffsetY)/tileSize == y)){
                //     ctx.globalAlpha = 0.7;
                // }
                var tileX = Math.floor((mouseX-boardOffsetX)/tileSize);//gets what tile the player clicked
                var tileY = Math.floor((mouseY-boardOffsetY)/tileSize);

                if(y==tileY && x==tileX && minesweeperStarted)ctx.globalAlpha = 0.3;

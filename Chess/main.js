...
//game startup
createBoard();//creates the board as an 8x8 grid
setupBoard(0,0);//sets pieces on the board
generateDeck();//generates the deck
//creates a save state
if(savedBoards.length > amountSavedBoards){
    savedBoards.shift();
}
savedBoards.push(new GameState(board, activeTeam, deck, currentCard, mineChaining, drawArrows, minesweeperStarted))
...
// function startup(){
//     var variantSelectorSize = 500;

//     if(mouseDown){
//         clearInterval(startUpLoop);
//         setInterval(main,30);//runs the main game loop every 30 ms
//     }else{
//         ctx.fillRect(canvas.width/2 - variantSelectorSize/2,100,variantSelectorSize,100);
//     }
// }
...
function findOpenTiles(){
    ...
    for(var y = 0; y < board.length; y++){
        for(var x =0; x < board[y].length; x++){
            if(board[y][x].piece == null && y>=(boardSizeY/2)-2){//only places pieces in the center of the board and on the current team's side
                openSpaces.push([x,y]);
            }
        }
    }
...
function main(){//the main game loop, runs every 30 ms
    ...
    if(minesweeperStarted){
        propagateMinesweeper();
        checkMinesweeperExplosion();
    }
...
function runPromotionScreen(){//displays and utalizes the promotion screen
    if(promotionScreenAcrive){//if the promotion is active, run this.
        var promotables = [Queen,Knook,Bishop,Rook,Knight,Antipawn];
        ...
        var renderSize = 150;
        var renderOffset = (canvas.width/2 - renderSize*promotables.length/2);

        if (!mouseDown) canSelectPromotion = true;//only allow the player to select if the mouse has raised

        ctx.fillStyle = highlightColor;
        ctx.globalAlpha = highlightAlpha;
        if(mouseX>=renderOffset && mouseX<promotables.length*renderSize + renderOffset){
            //filling the highlighted tiles
            ...
            if(mouseDown && canSelectPromotion){
                ...
                // }
                board[pieceToPromote[1]][pieceToPromote[0]].piece = new promotables[Math.floor((mouseX-renderOffset)/renderSize)](activeTeam);//promoting the piece to the selected piece
                //board[pieceToPromote[1]][pieceToPromote[0]].piece.team = activeTeam;//sets the active team of the piece to the active team
        ...
        for(var i =0;i<promotables.length;i++){
            //console.log(promotables[i]);
            let renderedPiece = new promotables[i];
            renderedPiece.team = activeTeam;
            
            ctx.drawImage(renderedPiece.sprite,(renderSize*i)+renderOffset,renderYOffset,renderSize,renderSize)
        }
...
function passTurn(){//called whenever a player makes a move. 
    //switching team
    canDraw = true;
    if(promotionScreenAcrive)return;//immediately exit if the promotion menu is active
...
function runUndo(){
    // if((key.control || key.meta)&&key.z){
    //     if(canUndo){
    //         if(savedBoards.length > 1){
    //             console.log(savedBoards[savedBoards.length-2].board);

    //             board = savedBoards[savedBoards.length-2].board;
    //             boardSizeX = savedBoards[savedBoards.length-2].boardSizeX;
    //             boardSizeY = savedBoards[savedBoards.length-2].boardSizeY;

    //             savedBoards.pop()

    //         }else{
    //             console.log("no saved gamestates")
    //         }
